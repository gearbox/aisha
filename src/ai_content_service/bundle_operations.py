...
async def run_removal(
    ...
    *,
    ...
    remover = BundleRemover(
        settings,
        residency=residency or ResidencyStore(settings.residency_path),
        workflow_manager=workflow_manager or WorkflowManager(settings.comfyui_path),
    )
    async with AsyncExitStack() as stack:
        if owns_reporter:
            await stack.enter_async_context(active_reporter)
...
async def run_comfyui_restart(
    ...
    *,
    ...
    owns_reporter = reporter is None
    active_reporter = reporter or ProvisioningReporter.from_settings(settings)
    active_manager = manager or ComfyUIManager(
    ...
    active_residency = residency or ResidencyStore(settings.residency_path)
